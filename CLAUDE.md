...
- Docker image is built from **this repo** (`envs/Dockerfile.hail`, based on `hailgenetics/hail`) and published to Docker Hub as `<DOCKERHUB_USERNAME>/mei-lr-association-hail` (CI names each image `<repo-name>-<env-name>`). The WDL selects the tag via the `ImageTag` input (defaults to `latest`; pass a 7-char commit SHA to pin a specific build).
...
- The insertion span itself is a `query_sequence` slice (no reference coordinates apply to inserted bases), but the flanks are defined in **reference** coordinates around the insertion anchor: `ref_aligned_segments` walks the CIGAR and keeps only `M`/`=`/`X`-aligned query bases within that window, so an insertion elsewhere in the flank is skipped rather than padding it with extra read sequence, and a deletion just shortens the reported flank (as does running off the end of the read/contig).
...
```
docker build -f envs/Dockerfile.hail -t mei-lr-association .
```
