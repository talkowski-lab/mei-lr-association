...
def region_stats(seq, calls, threshold):
    ...
    n_5mc, m_5mc, mean_5mc = summarize(calls, "5mC", threshold)
    n_5hmc, m_5hmc, mean_5hmc = summarize(calls, "5hmC", threshold)
    return [
        len(seq), seq.count("C"),
...
def insertion_region(read, q0, q1, keep_codes, threshold):
    """Region stats for the insertion itself: pure query-coordinate span."""
    seq = read.query_sequence[q0:q1]
    calls = mods_in_span(read, q0, q1, keep_codes)
