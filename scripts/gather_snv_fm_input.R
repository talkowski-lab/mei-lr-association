...
## Terra QTL data table -- reading a local file here instead of re-querying
## Terra's AVTable API once per feature in a scatter. The id column is
## assumed to be the table's first column. Column naming otherwise differs
## slightly between QTL types in practice (e.g. pQTL tables use
## "SubsetDosages" where eQTL/sQTL use "GenotypeDosages"), so each field
## tries a short list of known aliases.

argv <- arg_parser("Look up fine-mapping input file URIs for one feature from a downloaded QTL data table") %>%
  add_argument("--avtable-file", help = "Local TSV of the full QTL data table, id column first") %>%
  add_argument("--gene-name", help = "Row ID to look up (matches the first column)") %>%
  parse_args()

qtl_table <- read_tsv(argv$avtable_file, show_col_types = FALSE)

id_col <- colnames(qtl_table)[1]

row <- qtl_table %>% filter(.data[[id_col]] == argv$gene_name)
if (nrow(row) != 1) {
  stop(glue("Expected exactly 1 row with {id_col} == '{argv$gene_name}', found {nrow(row)}"))
}
...
pick_col <- function(candidates) {
  ...
  if (length(present) == 0) {
    warning(glue("None of the expected columns ({paste(candidates, collapse=', ')}) found -- writing an empty value"))
    return("")
  }
  row[[present[1]]]
}
